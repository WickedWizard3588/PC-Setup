    ...
    return new Promise((resolve) => {
        questions('Do you want me to write a config.json for you?').then(async (answer) => {
            if(answer.charAt(0) == 'y') {
                ...
                if(typeof wsl != 'boolean') {
                    template = {
                        ...template,
                        "wsl": {
                            "default-version": wsl,
                            "upgrade": wsl == 2 ? true : false
                        }
                    }
                }
                if(typeof cmder != 'boolean') {
                    template = {
                        ...template,
                        "cmder": {
                            "finalDirectory": cmder[0],
                            "environmentVariables": cmder[1],
                            "global": cmder[2],
                            "context-menu": cmder[3]
                        }
                    }
                }
                if(typeof ffmpeg != 'boolean') {
                    template = {
                        ...template,
                        "ffmpeg": {
                            "finalDirectory": ffmpeg[0],
                            "environmentVariables": ffmpeg[1],
                            "global": ffmpeg[2]
                        }
                    }
                }
