...
const IntelDSAWithoutConfig = async () => {
    ...
    let install = await questions('Do you want to install Intel Driver and Support?');
    install = install.charAt(0) == 'y' ? true : false;
    if(!install) {
    ...
    await execedsync('choco install intel-dsa');
    log('info', 'Successfully installed Intel DSA');
    return true;
};
