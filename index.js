...
const commands = ['REG QUERY "HKLM\\System\\CurrentControlSet\\Control\\Session Manager\\Environment" /v Path', 'REG QUERY HKCU\\Environment /v Path'];
...
const installFFmpeg = (rl) => {
    ...
    get(url, (response) => {
        ...
        response
            .on('data', (data) => writeFileSync('ffmpeg.7z', data))
            .on('end', async () => {
                ...
                execedsync(`set FFmpegDirectory="${__dirname}\\${filename}"`).then(console.log);
            })
            .on('error', (err) => {
...
const myCustomSetup = async (userVars) => {
    ...
    execed(`setx PATH "${userVars}${tasksdirectory.trim()};${nirsoftDirectory.trim()};${sixtyfourbit.trim()};${debugging.trim()};${primeninetyfive.trim()};${memtest.trim()}"`) // Trim and set the variables, so that there are no whitespaces.
...
const over = new Promise((resolve) => { // Creates a new promise.
    commands.forEach((command) => {
        execedsync(command).then((datas) => { // We make use of the `datas` returned to us.
            ...
            data = [...data, datas.trim()]; // Sets the data array.
...
over.then(async () => {
    ...
    // We are setting %ConEmuDir% Variable here, instead of the Batch File, because `%` percentage in Batch File refers to a variable, but here, we need it as an Environment Variable name.
        ...
        .on('error', console.log);
    questions(require('readline').createInterface(process.stdin, process.stdout), 'Did you install FFmpeg? Type "Y" for Yes and "N" for No. \nIf you haven\'t installed, this will install it.');
